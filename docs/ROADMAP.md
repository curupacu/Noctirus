...
- **🔴 Estourar a cota do Gemini é bem mais provável do que parecia** → o número de ~1.500 req/dia acima é o limite *padrão* do free tier documentado pelo Google, mas rodando `backend/scripts/avaliar-triagem.js` de verdade (28/07) o projeto atual bateu num erro 429 informando `GenerateRequestsPerDayPerProjectPerModel-FreeTier, quotaValue: "20"` — **20 requisições/dia**, não 1.500, pro modelo `gemini-2.5-flash-lite` nesse projeto (provavelmente por causa de alguma restrição de cota nova do Google nesse tier/conta, não documentada por eles). Isso é sério: **qualquer demonstração ao vivo com mais de ~20 triagens no dia esgota a cota e todo o resto cai silenciosamente no fallback por regras** (o sistema não quebra — RNF003 segura isso — mas a apresentação deixa de mostrar a IA funcionando, sem nenhum aviso na tela). Mitigação: (1) rodar `npm run avaliar-triagem` (backend/) antes da apresentação pra confirmar a cota disponível no dia; (2) evitar ficar testando a triagem repetidamente horas antes da banca; (3) se der, verificar/trocar pra uma chave de API nova ou investigar upgrade de tier no Google AI Studio antes do dia 13/08. O `console.error` que foi adicionado em `classificar()` (antes o erro da IA era engolido em silêncio) ajuda a confirmar isso pelos logs do Render.
